...
class Node_PChildren {
    ...
    createCopy(top_node_copy, instance_keys)
    {
        ...
        js0.implementsE(this.__main, Node.PCopyable);
        ...
        while(node_copies_stack.length > 0) {
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                if (!js0.implements(child_node_copy, Node.PChildren))
                    continue;

                // /* Copy only first repeat node. */
                // console.log('Test', node_copy === top_node);
                if (js0.var(child_node_copy, require('./nodes/RepeatNode')))
                    continue;
    ...
    deleteCopies(top_original_node, instance_keys)
    {
        ...
        while(original_nodes_stack.length > 0) {
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                if (!js0.implements(child_node, Node.PChildren))
                    continue;
    ...
    findNext(child_node)
    {
        ...
        js0.assert(child_node_index !== -1, '`child_node` not found.');
